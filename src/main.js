...
const app = {
  ...
  cellIssues: null, // 셀 단위 위반 주석 (cellIssues())
  showViol: true,   // 위반 표시 켜기/끄기
};
...
async function runSolve(warmUnits = null) {
  ...
  $('runLog').textContent = '';
  setPenalty('…', '생성 중', 'run');
  // 경과 시간 — 렉이 아니라 계산 중임을 1초 단위로 보여준다
  const runT0 = Date.now();
  $('elapsedTime').hidden = false;
  $('elapsedTime').textContent = '경과 0:00';
  clearInterval(app.elapsedTimer);
  app.elapsedTimer = setInterval(() => {
    const s = Math.floor((Date.now() - runT0) / 1000);
    $('elapsedTime').textContent = `경과 ${Math.floor(s / 60)}:${String(s % 60).padStart(2, '0')} — 계산 진행 중`;
  }, 1000);
  ...
  try {
    ...
    // 워커 수: CPU 코어에 맞추되, 메모리가 작은 기기(8GB 이하)는 8개로 제한해
    // WASM 힙 팽창으로 탭이 강제 종료되는 것을 줄인다.
    const memCap = (navigator.deviceMemory && navigator.deviceMemory <= 8) ? 8 : 16;
    const workers = Math.max(2, Math.min(navigator.hardwareConcurrency || 4, memCap));
...
function updateResultSummary() {
  ...
  app.cellIssues = app.lastState ? app.lastState.cellIssues() : new Map();
  const el = $('resultSummary');
...
// 위반 표시 켜기/끄기
$('btnViol').onclick = () => {
  app.showViol = !app.showViol;
  $('btnViol').textContent = app.showViol ? '위반 표시 켬' : '위반 표시 끔';
  $('btnViol').classList.toggle('on', app.showViol);
  renderPreview();
};
...
/** 위반 표시(켜져 있으면): 해당 셀의 클래스·title 속성 문자열 */
function violAttr(key) {
