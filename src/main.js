...
function afterDataLoaded(name) {
  ...
  for (const id of ['secNonclass', 'secUnavail', 'secSimilar', 'secRules', 'secParams']) {
    $(id).hidden = false;
  }
  $('railRun').hidden = false;
  revealNav(['secNonclass', 'secUnavail', 'secSimilar', 'secRules', 'secParams']);
  setPenalty('—', '준비됨', 'idle');
  renderAll();
  onUaTeacherChange();
}
...
$('btnTemplate').onclick = async () => {
  ...
  a.download = `시수표양식_${counts.join('-')}반_특별실${specials.length}.xlsx`;
  a.click();
...
$('btnSaveSettings').onclick = () => {
  downloadJson(collectSettings(), `시간표설정_${$('year').value}_${$('semester').value}.json`);
};
...
$('btnSession').onclick = () => {
  ...
  downloadJson(session, `이어돌리기_${$('year').value}_${$('semester').value}.json`);
};
...
$('sessionInput').onchange = async (e) => {
  ...
  try {
    ...
    if (app.sessionWarm) {
      ...
      $('fileInfo').innerHTML += ' — <b>이어돌리기 준비됨</b> ([이어서 더 돌리기]를 누르면 저장된 시간표에서 이어갑니다)';
    }
...
async function runSolve(warmUnits = null) {
  ...
  try {
    ...
    if (app.lastState) { $('btnMore').hidden = false; $('btnSession').hidden = false; }
  }
}
...
document.querySelectorAll('#viewSeg .seg-btn').forEach(btn => {
  btn.onclick = () => {
    ...
    $('viewMode').value = btn.dataset.view;
    updateViewTargets();
    renderPreview();
  };
});
...
(() => {
  ...
  board.addEventListener('drop', (e) => {
    ...
    const uB = picked.keyType === 'class' ? st.classUnitAt(picked.key, d, p) : st.teacherUnitAt(picked.key, d, p);
...
$('btnDownload').onclick = async () => {
  ...
  a.download = `시간표_${$('year').value}_${$('semester').value}_${ts}.xlsx`;
  a.click();
  setTimeout(() => URL.revokeObjectURL(a.href), 5000);
};
...
// 네비 클릭 → 부드러운 스크롤 (숨김 섹션 방지)
document.querySelectorAll('.nav-item').forEach(link => {
  link.addEventListener('click', (e) => {
    const sec = $(link.dataset.target);
    if (!sec || sec.hidden) { e.preventDefault(); }
  });
});

// 스크롤스파이: 현재 보이는 섹션을 네비에 강조
...
['secFile', 'secNonclass', 'secUnavail', 'secSimilar', 'secRules', 'secParams', 'secResult']
  .forEach(id => { const el = $(id); if (el) spy.observe(el); });
